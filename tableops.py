...
def df_aggregate_by_year(df, value_column, method='sum', date_column='Date'):

    if isinstance(value_column, str) and isinstance(method, str):
        result = df.groupby(df[date_column].dt.to_period('Y'))[value_column].agg(method).to_frame()
    elif isinstance(value_column, list) and isinstance(method, list):
        result = df.groupby(df[date_column].dt.to_period('Y')).agg({v:m for (v,m) in zip(value_column, method)})
    else:
        raise TypeError("'value_column' and 'method' arguments both have to be strings or both have to be lists")
    result = result.reset_index()
    result = result.set_index(['Date'])
